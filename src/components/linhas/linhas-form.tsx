    ...
    const handdleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!imagePreview) {
            const mensagemErro = isEditing
                ? "A Linha não pode ficar sem imagem. Selecione uma nova imagem ou cancele as alterações."
                : "Para cadastrar uma nova linha, é necessário selecionar uma imagem.";
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="md:max-w-lg bg-card text-card-foreground p-6">
                <DialogHeader>
                    <DialogTitle>{isEditing ? "Editar Linha" : "Nova Linha"}</DialogTitle>
                    <DialogDescription>
